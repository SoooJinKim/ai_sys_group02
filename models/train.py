...
import math
from models.inpainting_model2 import InpaintingModel
import torch
import gc
#tensorboard serve --logdir="C:\Users\Serin Kim\workspace\AISYS\DMFN-master\logs"

parser = argparse.ArgumentParser()
parser.add_argument('--config', type=str, default='configs/celeba-hq-regular_list2.yaml', help='path to the config file.')
...
def collate_fn(batch):
    ...
    # Free unused variables and perform garbage collection
    del batch
    gc.collect()
...
for phase, dataset_opt in config['datasets'].items():
    if phase == 'train':
        train_set = create_dataset(dataset_opt)
        train_size = int(math.ceil(len(train_set) / dataset_opt['batch_size']))
        print(f'Number of training images: {len(train_set):,d}, iters: {train_size:,d}')
        total_iters = int(dataset_opt['n_iter'])
        total_epochs = int(math.ceil(total_iters / train_size))
        print(f'Total epochs needed: {total_epochs} for iters {total_iters:,d}')
        train_loader = create_dataloader(train_set, dataset_opt, collate_fn=collate_fn)
    elif phase == 'val':
        val_set = create_dataset(dataset_opt)
        val_loader = create_dataloader(val_set, dataset_opt, collate_fn=collate_fn)
        print(f'Number of val images in [{dataset_opt["name"]}]: {len(val_set)}')
    elif phase == 'test':
        pass
    else:
        raise NotImplementedError(f'Unsupported phase: {phase}')

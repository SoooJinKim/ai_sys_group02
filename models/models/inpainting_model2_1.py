...
class InpaintingModel(BaseModel):
    ...
    def feed_data(self, data):
        self.var_overlay = data['input'].to(self.device)
    ...
    def optimize_parameters(self):
        ...
        self.output = self.var_mask.detach() * self.netG(torch.cat([self.var_overlay, self.var_mask], dim=1)) + (
                1 - self.var_mask.detach()) * self.var_overlay.detach()
        l_g_total = 0
        ...
        pred_g_fake, dis_feas_fake = self.netD(self.var_mask * self.output + (1 - self.var_mask) * self.var_overlay, self.output)
        pred_g_real, dis_feas_real = self.netD(self.var_mask * self.var_target + (1 - self.var_mask) * self.var_overlay, self.var_target)
        pred_g_real.detach_()
        ...
        l_d_total = 0
        pred_d_real, _ = self.netD(self.var_mask * self.var_target + (1 - self.var_mask) * self.var_overlay, self.var_target)
        pred_d_fake, _ = self.netD(self.var_mask * self.output + (1 - self.var_mask) * self.var_overlay.detach(), self.output.detach())
    ...
    def get_current_visuals(self):
        ...
        out_dict['input'] = self.var_overlay.detach()[0].float().cpu()
